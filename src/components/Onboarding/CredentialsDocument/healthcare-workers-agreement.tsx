...
export const HealthcareWorkersAgreement = () => {
  ...
  const onSubmit = () => {
    if (!isCheck) return;
    if (isCheck && principal?.userProfile?.healthCareWorkersAgreementAt) {
    ...
    setLoading(true);
    setAgreementToRequiredPolicies({ healthCareWorkersAgreement: true })
      .then(() => {
  ...
  const onPrevious = () =>
    changeQueryParams((params) => params.set('todo', 'arbitration-agreement'));
  useEffect(() => {
    if (principal?.userProfile?.healthCareWorkersAgreementAt) {
      setIsCheck(true);
      setIsAlreadyCheck(true);
    }
  }, [principal?.userProfile?.healthCareWorkersAgreementAt]);
...
export default HealthcareWorkersAgreement;
